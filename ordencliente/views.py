...
def editar_orden(request, id_servicio):
	...
	if request.method == 'GET':
		...
		ordenform = ClienteForm()
	return render_to_response('registrar_spot.html', {'servicio': servicio, 'ordenform': ordenform,'variable':'1'}, context_instance=RequestContext(request))
...
def editar_usuario(request, id_cliente):
	...
	return render_to_response('registrar.html', {'cuenta':cuenta, 'form':form}, context_instance=RequestContext(request))

@login_required
def lista_usuarios(request):
	cliente = clientes.objects.filter(usuario__is_active=True, usuario__is_staff=False).order_by('nombre')
...
def lista_usuarios_admin(request):
	...
	return render_to_response('usuarios_admin.html', {'admin_usuario':admin_usuario,}, context_instance=RequestContext(request))
...
def lista_spots(request):
	...
	return render_to_response('principal.html', {'usuario_staff':usuario_staff,'cambios_spot_staff':cambios_spot_staff,'usuario_admin':usuario_admin, 'servicio':servicio, 'usuarios': usuarios}, context_instance=RequestContext(request))
...
def detalle_spot(request, id_servicio):
	...
	return render_to_response('detalles_spots.html', {'dato': dato, 'historia':historia}, context_instance=RequestContext(request))

@login_required
def filtro_usuario(request, id_usuario):
	usuarios = User.objects.all().order_by('username')	
	usuario_admin = servicios.objects.filter(usuario=id_usuario)
	return render_to_response('principal.html', {'usuario_admin': usuario_admin, 'usuarios':usuarios, 'id_usuario':id_usuario}, context_instance=RequestContext(request))
...
def busqueda(request):
	...
	return render_to_response('busqueda.html', {'consulta':consulta,'fecha_1':fecha_1,'fecha_2':fecha_2 ,'buscar':buscar,'historia':historia}, context_instance=RequestContext(request))
...
def reporte_general_pdf(request):
	...
	if buscar:	
		...
		for h in consulta:
			historia = historial.objects.filter(servicio__usuario=h.usuario)
	return render_to_pdf('reporte_general_pdf.html', {'pagesize':'A4','fecha_1':fecha_1,'fecha_2':fecha_2, 'consulta':consulta, 'historia':historia})
